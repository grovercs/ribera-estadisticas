...
echo "=== IMPACT OF EMITIDO STATUS ON PENDING INVOICES ===\n";
...
$res = $db->select("
    ...
    GROUP BY f.cod_almacen, v.emitido
");

foreach ($res as $r) {
    echo "Store: " . ($r->cod_almacen ?? 'NULL') . " | Emitido: " . ($r->emitido ?? 'NULL') . " | Count: {$r->cnt} | Sum: " . number_format($r->sum_pend, 2) . "\n";
}
