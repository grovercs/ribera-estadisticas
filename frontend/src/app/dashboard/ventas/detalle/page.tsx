...
export default async function SalesDetailPage({ searchParams }: PageProps) {
  ...
  const singleDatePeriod = period === 'today' || period === 'yesterday'
  const hasValidSingleDate = singleDatePeriod && isValidDate(params.date)
  const hasValidFortnight = period === 'current_fortnight'
    && isValidDate(params.start)
    && isValidDate(params.end)
    && params.start <= params.end

  if (!hasValidSingleDate && !hasValidFortnight) {
    return (
  ...
  const startDate = hasValidSingleDate ? params.date! : params.start!
  const endDate = hasValidSingleDate ? params.date! : params.end!
  const periodTitle = period === 'today'
    ? 'Hoy'
    : period === 'yesterday'
      ? 'Ayer'
      : `Quincena actual: ${formatDate(startDate)} – ${formatDate(endDate)}`

  const supabase = await createClient()
  const from = `${startDate}T00:00:00`
  const to = `${nextDay(endDate)}T00:00:00`
    ...
    <div className="mx-auto w-full max-w-[1600px] space-y-4 text-[#191c1e]">
      <header className="flex flex-col gap-3 border-b border-[#e1e2e6] pb-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm font-medium text-[#747878]">{periodTitle}</p>
        </div>
      </header>
