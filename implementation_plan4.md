...
*   **Tablas clasificadas**:
    ...
    *   `stats_sellers`
    *   `products_stock`
*   **Estrategia de Clave Primaria**: La columna `batch_id` se incorpora al **inicio de la Clave Primaria Compuesta** para optimizar el indexado físico (B-Tree) en PostgreSQL.
...
*   **Estrategia de Auditoría en ERP (SQL Server)**:
    Se ha comprobado de forma expresa mediante consulta en el ERP de producción que tanto la tabla **`clientes`** como **`proveedores`** disponen del campo de auditoría **`fecha_modificacion (datetime)`** y **`fecha_hora_alta (datetime)`**. Por lo tanto, se implementará la estrategia incremental nativa para todas las tablas incrementales.
*   **Estrategia de Actualización**:
    *   Se realiza un `UPSERT` directo basado en la clave única natural.
    *   Se utiliza la columna `fecha_modificacion` de SQL Server para consultar únicamente los registros modificados recientemente (`WHERE fecha_modificacion >= ultima_sync_exitosa - 5 minutos`).
    *   **Campos de auditoría incluidos**:
        *   `source_modified_at`: Fecha y hora de modificación en el ERP.
...
#### C. Estrategia para `stats_sales_monthly` (Agregados Históricos)
Dado que esta tabla almacena los consolidados por año y mes:
*   Cuando la sincronización incremental detecte que una venta histórica ha sido modificada (`fecha_modificacion` en el ERP indica cambios en fechas pasadas), **se recalcularán completamente los totales agregados del mes y año afectados** en SQL Server.
*   Posteriormente, se realizará un **`UPSERT` del registro agregador** `(year, month)` en Supabase con los nuevos totales consolidados.
*   **Esta estrategia evita aplicar deltas aritméticos** que pueden acumular errores y garantiza consistencia absoluta e inmediata con el ERP.
...
El esquema SQL ha sido exhaustivamente auditado para asegurar consistencia total en tipos de datos, llaves primarias y referencias de índices:
...
-- KPIs Consolidados (batch_id primero en PK para indexado B-Tree óptimo)
...
Para asegurar la fluidez inmediata del panel web (Next.js) al consumir los endpoints de la API de Supabase, se configurarán los siguientes índices optimizados (corregido la referencia a `pending_amount` y agregando el filtro indexado de `batch_id` para las tablas snapshot):
